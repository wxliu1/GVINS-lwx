    ...
    const Eigen::Vector3d &rough_anchor_ecef, double &aligned_yaw, double &rcv_ddt)
{
    ...
    {
        // Eigen::MatrixXd在文档中的描述: Dynamic×Dynamic matrix of type double
        // 这里表示以num_all_meas为行，2为列构建动态矩阵
        // num_all_meas为GNSS数据观测数据总个数，2为优化量个数:钟差变化率和yaw角
        ...
        for (uint32_t i = 0; i < gnss_meas_buf.size(); ++i)
        {
            ...
            align_G.block(align_counter, 0, gnss_meas_buf[i].size(), 1) = 
                epoch_J.leftCols(3)*rough_R_ecef_enu*align_tmp_M*local_vs[i]; // 因为dopp_res中仅仅对速度求导，所以需要乘上速度到偏航角的导数，即链式求导得到多普勒测量对偏航角求导
